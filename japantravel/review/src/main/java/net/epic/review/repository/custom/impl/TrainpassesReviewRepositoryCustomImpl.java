...
import net.epic.review.entity.TrainpassesTypeRatings;
...
/**
 * Implementation Class for the  Trainpasses Review Repository Custom
 * @author uditha
 */
public class TrainpassesReviewRepositoryCustomImpl implements TrainpassesReviewRepositoryCustom {
    ...
    public TrainpassesReview saveReviewWithStars(TrainpassesReviewDTO reviewDTO) {
        ...
        TrainpassesReview review = new TrainpassesReview();
        review.setComment(reviewDTO.getComment());
        review.setUsername(reviewDTO.getUsername());
        review.setDatetime(new Date());
        review.setTrainpassesId(1);
        ...
        TrainpassesTypeRatings typeRatings;
        ...
        if (reviewDTOlist != null) {
            for (CommenStarReviewDTO commenDTO : reviewDTOlist) {
                typeRatings = new TrainpassesTypeRatings();
                typeRatings.setTrainpassesReviewId(review);
    ...
    public List<CommenStarReviewDTO> getAllStarRatings(Integer trainpassesId) {
        ...
        Criteria cc = session.createCriteria(TrainpassesReview.class).add(Restrictions.eq("trainpassesId",trainpassesId));
        if(cc.list().size()==0){
            totalReviews = 1;
        }else{
        ...
        List<ReviewTypes> reviewTypesList = session.createCriteria(ReviewTypes.class).add(Restrictions.eq("category", "TRAINPASSES")).list();
        for (ReviewTypes types : reviewTypesList) {
            ...
            cr = session.createCriteria(TrainpassesTypeRatings.class, "de").createCriteria("de.trainpassesReviewId", "li");
            cr.add(Restrictions.eq("de.reviewTypeId", types));
            cr.add(Restrictions.eq("li.trainpassesId", trainpassesId));
    ...
    @Autowired
    public TrainpassesReviewRepositoryCustomImpl(EntityManager entityManager) {
