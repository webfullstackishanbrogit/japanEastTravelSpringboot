...
import net.epic.review.entity.TransportationTypeRatings;
...
/**
 * Implementation Class for the  Transportation Review Repository Custom
 * @author uditha
 */
public class TransportationReviewRepositoryCustomImpl implements TransportationReviewRepositoryCustom {
    ...
    public TransportationReview saveReviewWithStars(TransportationReviewDTO reviewDTO) {
        ...
        TransportationReview review = new TransportationReview();
        review.setComment(reviewDTO.getComment());
        review.setUsername(reviewDTO.getUsername());
        review.setDatetime(new Date());
        review.setTransportationId(1);
        ...
        TransportationTypeRatings typeRatings;
        ...
        if (reviewDTOlist != null) {
            for (CommenStarReviewDTO commenDTO : reviewDTOlist) {
                typeRatings = new TransportationTypeRatings();
                typeRatings.setTransportationReviewId(review);
    ...
    public List<CommenStarReviewDTO> getAllStarRatings(Integer transportationId) {
        ...
        Criteria cc = session.createCriteria(TransportationReview.class).add(Restrictions.eq("transportationId",transportationId));
        if(cc.list().size()==0){
            totalReviews = 1;
        }else{
        ...
        List<ReviewTypes> reviewTypesList = session.createCriteria(ReviewTypes.class).add(Restrictions.eq("category", "TRANSPORTATION")).list();
        for (ReviewTypes types : reviewTypesList) {
            ...
            cr = session.createCriteria(TransportationTypeRatings.class, "de").createCriteria("de.transportationReviewId", "li");
            cr.add(Restrictions.eq("de.reviewTypeId", types));
            cr.add(Restrictions.eq("li.transportationId", transportationId));
    ...
    @Autowired
    public TransportationReviewRepositoryCustomImpl(EntityManager entityManager) {
