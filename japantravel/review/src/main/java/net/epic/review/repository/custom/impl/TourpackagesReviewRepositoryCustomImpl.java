...
import net.epic.review.entity.TourpackagesTypeRatings;
...
/**
 * Implementation Class for the  Tourpackages Review Repository Custom
 * @author uditha
 */
public class TourpackagesReviewRepositoryCustomImpl implements TourpackagesReviewRepositoryCustom {
    ...
    public TourpackagesReview saveReviewWithStars(TourpackagesReviewDTO reviewDTO) {
        ...
        TourpackagesReview review = new TourpackagesReview();
        review.setComment(reviewDTO.getComment());
        review.setUsername(reviewDTO.getUsername());
        review.setDatetime(new Date());
        review.setTourpackagesId(1);
        ...
        TourpackagesTypeRatings typeRatings;
        ...
        if (reviewDTOlist != null) {
            for (CommenStarReviewDTO commenDTO : reviewDTOlist) {
                typeRatings = new TourpackagesTypeRatings();
                typeRatings.setTourpackagesReviewId(review);
    ...
    public List<CommenStarReviewDTO> getAllStarRatings(Integer tourpackagesId) {
        ...
        Criteria cc = session.createCriteria(TourpackagesReview.class).add(Restrictions.eq("tourpackagesId",tourpackagesId));
        if(cc.list().size()==0){
            totalReviews = 1;
        }else{
        ...
        List<ReviewTypes> reviewTypesList = session.createCriteria(ReviewTypes.class).add(Restrictions.eq("category", "TOURPACKAGES")).list();
        for (ReviewTypes types : reviewTypesList) {
            ...
            cr = session.createCriteria(TourpackagesTypeRatings.class, "de").createCriteria("de.tourpackagesReviewId", "li");
            cr.add(Restrictions.eq("de.reviewTypeId", types));
            cr.add(Restrictions.eq("li.tourpackagesId", tourpackagesId));
    ...
    @Autowired
    public TourpackagesReviewRepositoryCustomImpl(EntityManager entityManager) {
